...
jQuery(function($){
    ...
    $('.wc-mesomb-form input[name=billing_phone]').on('change', function (evt) {
        $('#mesomb-payer').val(evt.target.value)
    })
    $('body').on('change', '.wc-mesomb-form input[name=country]', function (evt) {
    ...
    $('body').on('change', '.wc-mesomb-form input[name=service]', function (evt) {
        const service = evt.target.value;
        if (service) {
            $('#mesomb-payer').attr('placeholder', placholders[service])
        }
    });
    $('.wc-mesomb-form form[name=checkout]').on('submit', function (evt) {
        $('#mesomb-alert').show();
